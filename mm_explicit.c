...
block_t *root; //pointer to first block in free list
...
//function to check free list for debugging
void check_freelist()
{
  block_t *b;
  //iterate through the loop and print out the free blocks;
  for (b = (void*)root; b->body.next != NULL; b = (void *)b->body.next) {
        printblock(b);
        printf("next = (%p); prev = (%p)\n", (void *)b->body.next, (void *)b->body.prev);
  }
}
...
static void deleteFromFreeList(block_t *block)
{
  ...
  //if the block is the last element in list, set the previous element's next ptr to NULL
    ...
    block_t *temp = (void *)block->body.prev;
    temp->body.next = NULL;
  }
  ...
  //previous pointer to NULL
    ...
    block_t *temp = (void *)block->body.next;
    root = (void *)temp;
    temp->body.prev = NULL;
  }
  else{
