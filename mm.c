...
block_t **seg_list; //pointer to first block in free list
...
static void deleteFromFreeList(block_t *block);
static uint32_t get_offset(uint32_t asize);
void mm_checkheap(int verbose);
/* */
/* $begin mminit */
int mm_init(void) {
    seg_list = mem_sbrk(NUM_LISTS * 8);
    uint32_t offset = 0;
    for(; offset < NUM_LISTS; offset += 1)
    {
      seg_list[offset] = NULL;
    }
    ...
    //Initialize free list: link root to init_block
    addToFreeList(init_block);
    return 0;
}
/* $end mminit */
...
static block_t *place(block_t *block, size_t asize) {
    ...
    if (split_size >= MIN_BLOCK_SIZE && asize < 128) {
    ...
    else if(split_size >= MIN_BLOCK_SIZE && asize > 128)
    {
        /* update the header of the new free block */
        block_t *new_block = (void *)block;
        new_block->block_size = split_size;
        new_block->allocated = FREE;
        /* update the footer of the new free block */
        footer_t *new_footer = get_footer(new_block);
        ...
        deleteFromFreeList(block);
        /* split the block by updating the header and marking it allocated*/
        block = (void *)new_block + new_block->block_size;
        block->block_size = asize;
        block->allocated = ALLOC;
        /* set footer of allocated block*/
        footer_t *footer = get_footer(block);
 ...
 */
static block_t *find_fit(size_t asize) {
    uint32_t offset = get_offset(asize);
    block_t *b;
    for (; offset < (uint32_t)NUM_LISTS; offset += 1)
    {
      for (b = (void*)seg_list[offset]; b != NULL; b = (void *)b->body.next) {
...
static block_t *coalesce(block_t *block) {
    ...
    if (prev_alloc && next_alloc) { /* Case 1 */
        ...
        deleteFromFreeList((void *)next_header);
...
static void printblock(block_t *block) {
    ...
    printf("%p: header: [%d:%c] %p: footer: [%d:%c]\n", block, hsize,
           (halloc ? 'a' : 'f'), footer, fsize, (falloc ? 'a' : 'f'));
}
...
//add block to the start of free list
static void addToFreeList(block_t *block)
{
  uint32_t offset = get_offset(block->block_size);
  //if there is not element in the list, link it to root
  if(seg_list[offset] == NULL){
    block->body.prev = NULL;
    block->body.next = NULL;
    seg_list[offset] = (void *)block;
    return;
  }
  block->body.prev = NULL;
  block->body.next = (void *)seg_list[offset];
  seg_list[offset]->body.prev = (void *)block;
  seg_list[offset] = (void *)block;
}
...
{
  uint32_t offset = get_offset(block->block_size);
  block_t *previous = (void *)block->body.prev;
  block_t *next = (void *)block->body.next;
  //if the block is the only element in free list, set root to NULL.
  if(block->body.prev == NULL && block->body.next == NULL){
    seg_list[offset] = NULL;
  }
  //if the block is the last element in list, set the previous element's next ptr to NULL
  else if(block->body.next == NULL){
    previous->body.next = NULL;
  }
  //if the block is first in list, set root to the next element and next element's
  //previous pointer to NULL
  else if(block->body.prev == NULL){
    seg_list[offset] = (void *)next;
    next->body.prev = NULL;
  }
  else{
    //otherwise link the previous and next block together
    next->body.prev = (void *)previous;
    previous->body.next = (void *)next;
  }
  return;
}
...
static uint32_t get_offset(uint32_t asize)
{
  uint32_t base = 128;
  uint32_t offset = 0;
  while(offset < NUM_LISTS - 1 && asize > base)
  {
    base <<= 1;
    offset += 1;
  }
