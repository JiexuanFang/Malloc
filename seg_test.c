...
#define LIST_HEAD_NUM 20
static block_t *list_head[LIST_HEAD_NUM]; /* 指向 explicit list 头部的指针 */

/* function prototypes for explicit list */
static uint32_t list_get_head_offset(uint32_t size);
static void list_initialize_block(block_t *block);
static void list_link_block(block_t *prev_block, block_t *next_block);
static void list_replace(block_t *old_block, block_t *new_block);
static void list_insert(block_t *block);
static void list_delete(block_t *block);

uint32_t list_get_head_offset(uint32_t size) {
    uint32_t base = 32, offset = 0;
    while (size > base && offset + 1 < LIST_HEAD_NUM) {
        offset += 1;
        base <<= 1;
    }
    return offset;
...
void list_initialize_block(block_t *block) {
    block->body.prev = NULL;
    block->body.next = NULL;
}

void list_link_block(block_t *prev_block, block_t *next_block) {
    if (prev_block != NULL)
        prev_block->body.next = (void *)next_block;
    if (next_block != NULL)
        next_block->body.prev = (void *)prev_block;
}

void list_insert(block_t *block) {
    list_initialize_block(block);
    uint32_t offset = list_get_head_offset(block->block_size);
    if (list_head[offset] == NULL) {
        list_head[offset] = block;
        return;
    }
    block_t *next_block = list_head[offset];
    list_link_block(block, next_block);
    list_head[offset] = block;
}

void list_delete(block_t *block) {
    block_t *prev_block = (void *)block->body.prev;
    block_t *next_block = (void *)block->body.next;
    list_initialize_block(block);
    list_link_block(prev_block, next_block);
    uint32_t offset = list_get_head_offset(block->block_size);
    if (list_head[offset] == block)
        list_head[offset] = next_block;
}
...
int mm_init(void) {
    ...
    /* 初始化 explicit list */
    uint32_t offset;
    for (offset = 0; offset < LIST_HEAD_NUM; offset++)
        list_head[offset] = NULL;
    list_insert(init_block);
...
static void place(block_t *block, size_t asize) {
    ...
    puts("[In Place]");
    printf("asize: %d\n", (int)asize);
    printf("pointer or list head: %p\n", list_head);
    printf("pointer of block: %p\n", block);
    puts("[Out Place]");
    */
...
static block_t *find_fit(size_t asize) {
    /* first fit search */
    uint32_t offset = list_get_head_offset((uint32_t)asize);
    block_t *b;
    // mm_checkheap(1);
    for (; offset < (uint32_t)LIST_HEAD_NUM; offset++) {
        for (b = list_head[offset]; b != NULL; b = (void *)b->body.next) {
